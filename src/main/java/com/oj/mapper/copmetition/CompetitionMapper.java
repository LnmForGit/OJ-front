...
public interface CompetitionMapper {
    ...
    public int getSum();
    //
    @Select("select id,name,FROM_UNIXTIME(start) as start,FROM_UNIXTIME(end) as end,description\n" +
    ...
    @Select("select id,name,FROM_UNIXTIME(start) as start,FROM_UNIXTIME(end) as end,description from teach_test where id=#{id} ")
    ...
    //用户未参加过的
    @Select("select id from teach_test where kind=4 and id not in(select DISTINCT test_id from teach_submit_code where user_id=#{user_id})")
    ...
    @Select("select id from teach_test where kind=4")
